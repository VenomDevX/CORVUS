...
Ollama 500s that mention allocation ("cudaMalloc failed", "unable to allocate CUDA_Host buffer", "failed to allocate CPU_REPACK buffer") are usually the **Windows commit limit**, not free RAM and not VRAM — Windows grants allocations against commit, so a 3GB request fails instantly while physical RAM sits free. The pagefile is system-managed now (July 2026; it was manually capped, which pinned the commit limit near what a full desktop already charges), but 16GB RAM with several browsers plus an IDE can still exhaust commit. Diagnose with `(Get-CimInstance Win32_OperatingSystem).FreeVirtualMemory` (commit available), *not* `FreePhysicalMemory`; the buffer named in the error just says which allocator hit the wall first. Closing a browser frees GB-scale commit. The 6GB RTX 4050 is a real ceiling too — `qwen2.5-coder` needs ~3.7GB VRAM for weights alone.
